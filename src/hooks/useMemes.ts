...
export function useMemes() {
  ...
  const toggleFavorite = async (memeId: string) => {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        if (!user) {
          toast.error('Você precisa estar logado para favoritar memes');
          return;
        }
    ...
    // Salvar no localStorage se não houver backend
    ...
    setFeaturedMemes(prev => prev.map(meme => 
      meme.id === memeId 
        ? { ...meme, favorite_count: meme.favorite_count + (isFavorited ? -1 : 1) }
        : meme
    ));
  };
  ...
  const downloadMeme = async (meme: Meme) => {
    try {
      ...
      setFeaturedMemes(prev => prev.map(m => 
        m.id === meme.id 
  ...
  const checkForDuplicates = async (file: File): Promise<boolean> => {
    ...
    try {
      ...
      const fileNameBase = fileName.replace(/\.[^/.]+$/, "").replace(/[^a-z0-9]/g, '');
      const hasSimilarMatch = data?.some(meme => {
        const memeTitle = (meme.title || '').toLowerCase().replace(/[^a-z0-9]/g, '');
        const memePath = (meme.image_path || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  ...
  const uploadMeme = async (file: File, metadata: {
    title: string;
    description?: string;
    tags: string[];
    category: string;
    ...
    if (!isSupabaseConfigured || !supabase) {
      toast.error('Backend não configurado. Configure o Supabase para fazer uploads permanentes.');
      return null;
    }
    ...
    try {
      ...
      if (!user) {
        toast.error('Você precisa estar logado para fazer upload de memes');
        return null;
      }
      ...
      if (existingCategory) {
        ...
        // Se a categoria não existir, usar "Outros" como fallback
  ...
  const searchMemes = async (query: string, category?: string) => {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        if (query) {
          queryBuilder = queryBuilder.or(`title.ilike.%${query}%,ocr_text.ilike.%${query}%,description.ilike.%${query}%`);
        }
